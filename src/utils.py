...
import requests
from schemas import HealthResponse, ErrorResponse, RetryResponse, SimplifiedDownloadResponse, UploadFastaResponse, StatusResponse
...
def test_health(url: str):
	...
	try:
		...
		if health_response.ok:
			...
			return ErrorResponse(error=f"Error: {health_response.status_code} {health_response.text}")
...
def download_zip(
	...
	headers = _get_auth_headers()
	try:
		response = requests.get(f"{url}/download/{run_id}", stream=True, headers=headers)
		if response.ok:
			...
			return SimplifiedDownloadResponse(run_id=run_id, destination_filepath=str(destination_filepath))
		else:
			logger.info("Analysis is likely still in progress!")
			return ErrorResponse(error=f"Error downloading file: {response.status_code} {response.text}")
	except Exception as e:
...
def retry_sequences(url: str, run_id: str):
	...
	try:
		...
		if response.ok:
			...
			return ErrorResponse(error=f"Error retrying sequences: {response.status_code} {response.text}")
	except Exception as e:
